...
const Langs = {
  ...
  lack: ({ s, items, mode }) => `${sub(s, s => `${s}の`)}${sub(items?.join("と") || undefined, s => `${s}を`)}${mode === "select" ? "選択" : mode === "set" ? "設定" : "入力"}してください`,
  ...
  maxLength: ({ s, maxLen, ...p }) => `${sub(s, s => `${s}は`)}${maxLen}文字以下で入力してください${writeHas(p, "cur", cur => `（現在：${cur}）`)}`,
  ...
  rangeDate: ({ s, minDate, maxDate }) => `${sub(s, s => `${s}は`)}${minDate}～${maxDate}の範囲で入力してください`,
  contextDate: () => `日付の前後関係が不適切です`,
  contextTime: () => `時間の前後関係が不適切です`,
  fileAccept: ({ s }) => `${sub(s, s => `${s}の`)}拡張子が不適切です`,
  fileSize: ({ s, size }) => `${sub(s, s => `${s}の`)}ファイルサイズは${size}以下で設定してください`,
